...
public class TilesUtil {
	...
	public static XYint coordinateToDisplay(double lat, double lng, int zoom, boolean isYandex){
		...
		if (isYandex) {
			...
			projection = Math.log(Math.tan(Math.toRadians(lat) / 2 + Math.PI / 4));
		}
	...
	public static LatLng displayToCoordinate(XYint point, int zoom, boolean isYandex) {
		double quarterPixelsAtZoom = (double)(1 << (zoom + (LOG2_TILESIZE - 2)));
		...
		if (isYandex) {
			...
			double latRad = Math.atan(Math.exp(yy / (quarterPixelsAtZoom * (-2 / Math.PI)))) * 2 - Math.PI / 2;
			...
			do {
				...
				latRad = Math.asin(1 - (sinPrev + 1) * Math.pow(1 - exct * sinPrev, exct) /
							(Math.exp(yy * k) * Math.pow(1 + exct * sinPrev, exct)));
			} while (Math.abs(prev - latRad) > 1e-9 && --i > 0);

			if (i >= 0 && !Double.isNaN(latRad))
				return new LatLng(Math.toDegrees(isLatNeg ? -latRad : latRad), lng);

			System.out.println("Failed to calculate lattitude for y=" + point.y + ", zoom=" + zoom);
		}
	...
	private static boolean waitForImageLoaded(Image img) {
		...
		if (img.getWidth(null) == TILESIZE
		    && img.getHeight(null) == TILESIZE
	 ...
	 */
	public static XYint[] drawTilesArea(int width, int height, XYint centerTileTopLeft, XYint tileXY, XYint tileWikiXY, Graphics dbf) {
		int tilesToLeft = centerTileTopLeft.x <= 0 ? 0 : (centerTileTopLeft.x + (TILESIZE - 1)) >> LOG2_TILESIZE;
		int tilesToRight = centerTileTopLeft.x + TILESIZE >= width ? 0 : (width - centerTileTopLeft.x - 1) >> LOG2_TILESIZE;

		int tilesToTop = centerTileTopLeft.y <= 0 ? 0 : (centerTileTopLeft.y + (TILESIZE - 1)) >> LOG2_TILESIZE;
		int tilesToBottom = centerTileTopLeft.y + TILESIZE >= height ? 0 : (height - centerTileTopLeft.y - 1) >> LOG2_TILESIZE;
		...
		XYint[] matrix = {
			new XYint((tileXY.x - tilesToLeft) << LOG2_TILESIZE, (tileXY.y - tilesToTop) << LOG2_TILESIZE),
		...
		if (Config.drawWikimapia){
			...
			int endY = Math.min(startY + totalMatrixH, Config.isMapYandex ? maxTileY(zoom, false) : maxY);
			if (startY < 0) {
				startY = 0;
			}
